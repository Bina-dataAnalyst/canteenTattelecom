...
import { Settings } from 'lucide-react'; // Не забудь установить lucide-react если еще нет

export const HeaderWidgets = () => {
    const [isMacrosOpen, setIsMacrosOpen] = useState(false);
    const { getTotalStats, dailyLimit, setDailyLimit } = useCartStore();
    ...
    const widgetClass = "relative w-70 h-30 bg-white/80 backdrop-blur-md rounded-tr-[40px] rounded-bl-[40px] shadow-lg p-4 flex flex-col justify-center items-center transition-all hover:scale-105 border border-white/50";
  
    return (
      <>
        <div className="sticky top-6 flex justify-center gap-14 -mt-24 mb-1 relative z-50 animate-in fade-in slide-in-from-top-4 duration-700">
          
          {/* Виджет 1: Динамический счетчик */}
          <div className={widgetClass}>
            <p className="text-[18px] text-gray-500 font-bold uppercase mb-1 tracking-wider">Счетчик калорий</p>
            
            <div className="flex items-center gap-2">
                <div className={`px-6 py-1 rounded-full font-black text-white text-m transition-colors duration-500 ${isOver ? 'bg-red-500' : 'bg-orange-500'}`}>
                    {calories} ккал
                </div>
                ...
                {isEditing ? (
                    <input 
                        ...
                        autoFocus
                        className="w-14 bg-gray-100 rounded px-1 text-m font-bold focus:outline-none"
                    />
                ) : (
                    <div 
            ...
            <div className="w-32 h-1 bg-gray-100 mt-2 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.min((calories / dailyLimit) * 100, 100)}%` }}
              />
            </div>
          </div>

          {/* Виджет 2: Заказы */}
          <button className={widgetClass}>
            <p className="text-[18px] text-gray-500 font-bold uppercase mb-1">История заказов</p>
            <div className="bg-[#4CAF50] text-white px-8 py-1 rounded-full font-black text-m italic">Смотреть</div>
          </button>
